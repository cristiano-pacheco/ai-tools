...
1. **Project index** — `engineering/<project>/index.md`: read it (if missing, create it with `# <project>` and a `↑ [[engineering/index|Engineering]]` back-link); append a bullet `- [[engineering/<project>/code-reviews/<timestamp>-<branch>|<timestamp>-<branch>]]` under a `## Code Reviews` heading (each review is a new file, so always append).
