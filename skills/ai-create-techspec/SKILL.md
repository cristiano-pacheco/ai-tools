...
- Required input: the PRD at `engineering/<project>/<feature>/prd.md` (read it from the vault).
...
List `engineering/<project>` with `obsidian_list_files_in_dir` to find the feature folder; if ambiguous or missing, ask the user. Read the PRD with `obsidian_get_file_contents("engineering/<project>/<feature>/prd.md")`. If it's missing, stop and tell the user to run `ai-create-prd` first.

### Write the file (no whole-file overwrite tool)

To write `engineering/<project>/<feature>/tech-spec.md`: check existence with `obsidian_get_file_contents`, delete with `obsidian_delete_file` (pass `confirm: true`) if present, then create with `obsidian_append_content` (it creates missing parent folders).
...
Write to `engineering/<project>/<feature>/tech-spec.md` using the recipe above, then confirm the path.
...
- [ ] Saved to `engineering/<project>/<feature>/tech-spec.md`
