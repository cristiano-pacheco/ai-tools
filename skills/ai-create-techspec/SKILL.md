...
Use a concise message naming the note (e.g. `ai-create-techspec: <feature>`). If there's nothing staged, no `origin`, or the push fails (offline), report it briefly and finish — don't abort the skill. `ai-setup` configures the repo and its `origin`.
